...
def thresholdavg(abf, sweep, thresdvdt = 20):
    ...
    indexhigher = pyabf.tools.ap.ap_points_currentSweep(abf) #Returns indices only where the slope is greater than the threshold
    ...
    thresholdavghold = thresholdavghold[1:]
    l, = thresholdavghold.shape
    if l > 1:
        thresholdavg1 = np.mean(thresholdavghold)
    elif l == 1:
        thresholdavg1 = thresholdavghold[0]
    else:
        thresholdavg == nan
    print (thresholdavg1)
...
def appreprocess3(abf, tag = 'default', save = False, plot = False):
    apf = False
    apstrt = 0
    apend = 0
    ...
    for sweepNumber in range(sweepcount):
        ...
        thresholdsl = (thresholdavg(abf, sweepNumber))
