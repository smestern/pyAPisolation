...
def exp_growth_factor(dataT,dataV,dataI, alpha, end_index=1, plot=False):
    try:
        ...
        curve = curve_fit(exp_grow_2p, t1, dataV[upwardinfl:end_index], maxfev=50000, bounds=([-np.inf, (_height//2), 0, 0, 0], [0, _height*2, np.inf, np.inf, 0.5]), xtol=None)[0]
        tau = curve[2]
            ...
            if abf.sweepLabelY != 'Clamp Current (pA)' and abf.protocol != 'Gap free' and protocol_name in abf.protocol:
              ...
              #try:
                ...
                for sweepNumber in sweepList: 
                    ...
                    decay_fast, decay_slow, curve, r_squared_2p, r_squared_1p, p_decay = exp_decay_factor_alt(dataT, dataV, dataI, time_after, abf_id=abf.abfID)
                    ...
                    grow = exp_growth_factor(dataT, dataV, dataI, decay_slow)
