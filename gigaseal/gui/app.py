...
class MainWindow(QMainWindow):
    ...
    APP_TITLE = "gigaseal — Patch-Clamp Analysis"
    ...
    def _setup_menu_bar(self):
        ...
        try:
            from gigaseal.gui.wizard_integration import add_analysis_wizard_to_menu
        ...
        try:
            from gigaseal.dev.prism_writer_gui import PrismWriterGUI
    ...
    def _on_batch_progress(self, done: int, total: int):
        """Status-bar tick for batch progress."""
        if total > 0:
            self._status.showMessage(f"Batch: {done}/{total}")
    ...
    def _on_span_selected(self, xmin: float, xmax: float):
        ...
        if "start" not in current_vals and "end" not in current_vals:
            self._status.showMessage(
                "Span selection ignored: current module has no start/end params"
            )
            return
...
def main():
    ...
    app.setApplicationName("gigaseal")
    window = MainWindow()
    window.show()
    sys.exit(app.exec())
