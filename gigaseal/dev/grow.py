...
from gigaseal.loadABF import loadABF
...
def exp_growth_factor(dataT,dataV,dataI, end_index=300):
    #try:
        ...
        curve = curve_fit(exp_grow, t1, dataV[upwardinfl:end_index], maxfev=50000, p0=[upperC, diffC, 10])[0]
        curve2 = curve_fit(exp_grow_2p, t1, dataV[upwardinfl:end_index], maxfev=50000,method='trf', bounds=([upperC-5,  0, 0,  0,  -np.inf], [upperC+5, diffC+5, np.inf, diffC,np.inf]), xtol=None, gtol=None, ftol=1e-14, jac='3-point')[0]
        tau = curve[2]
        tau1 = 1/curve2[2]
        tau2 = 1/curve2[4]
        tau_idx = [2, 4]
        ...
        plt.plot(t1, exp_grow(t1, *curve[:3]), label=f'single phase', c='m', alpha=0.5)
